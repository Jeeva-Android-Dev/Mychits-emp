...
import com.mazenet.mani.mounika.Adapters.ComtmntPaymentReportList
...
class ComntPaymentReport : BaseFragment() {

    val customerlist = ArrayList<ComtmntPaymentModel>()
    val customerlist_showing = ArrayList<ComtmntPaymentModel>()
    lateinit var recycler_collreports: RecyclerView
    lateinit var collReportAdapte: ComtmntPaymentReportList
    ...
    override fun onCreateView(
        ...
        (activity as HomeActivity).setActionBarTitle("Commitment Report")
        ...
        txt_cr_col2.setText(resources.getString(R.string.rel_amnt))
        txt_cr_col3.setText(resources.getString(R.string.paymenttype))
        txt_disp_total.setText(resources.getString(R.string.tot_relsd_amnt))
            ...
            override fun onTextChanged(
                ...
                if (text == "" || text == null) {
                    ...
                    if (customerlist_showing.size > 0) {
                        ...
                        collReportAdapte =
                            ComtmntPaymentReportList(customerlist_showing, object : AdapterClickListener {
                                override fun onPositionClicked(view: View, position: Int) {
                                    if (view.getTag().equals("listitem")) {
                                        val bundle = Bundle()
                                        bundle.putString("enrolid", customerlist_showing.get(position).getEnrollmentId())
...
//                mobile = mobile.toLowerCase(Locale.getDefault())
        ...
        if (customerlist_showing.size > 0) {
            recycler_collreports.setVisibility(View.VISIBLE)
            collReportAdapte = ComtmntPaymentReportList(customerlist_showing, object : AdapterClickListener {
                override fun onPositionClicked(view: View, position: Int) {
                    if (view.getTag().equals("listitem")) {
                        val bundle = Bundle()
                        bundle.putString("enrolid", customerlist_showing.get(position).getEnrollmentId())
    ...
    fun get_Collectionlist(
        ...
        if (checkForInternet()) {
            showProgressDialog()
            println("fetching")
            var resultlist = java.util.ArrayList<ComtmntPaymentModel>()
            ...
            val LeadListRequest = getleadslist.get_comtpayment_reports(loginparameters)
            LeadListRequest.enqueue(object : Callback<ArrayList<ComtmntPaymentModel>> {

                override fun onFailure(call: Call<ArrayList<ComtmntPaymentModel>>, t: Throwable) {
                    hideProgressDialog()
                    t.printStackTrace()
                ...
                override fun onResponse(
                    call: Call<ArrayList<ComtmntPaymentModel>>,
                    response: Response<ArrayList<ComtmntPaymentModel>>
                ) {
                    hideProgressDialog()
                    when {
    ...
    private fun integrateList(leadslist: ArrayList<ComtmntPaymentModel>) {
        if (leadslist.size > 0) {
            customerlist.clear()
            customerlist_showing.clear()
            ...
            collReportAdapte = ComtmntPaymentReportList(customerlist_showing, object : AdapterClickListener {
                override fun onPositionClicked(view: View, position: Int) {
                    if (view.getTag().equals("listitem")) {
                        val bundle = Bundle()
                        bundle.putString("customerid", customerlist_showing.get(position).getCustomerId())
                        doFragmentTransactionWithBundle(CommitmentPaymentReportDetail(), "comttmntdetail", true, bundle)
                    } else if (view.getTag().equals("custname")) {
                    }
                }
            ...
            try {
                ...
                for (i in customerlist_showing) {
                    total += Constants.isEmtytoZero(i.getDueAmount()!!).toInt()
                }
