...
import com.mazenet.mani.mounika.Adapters.Adapter_businessagentReport
...
class BusinessAgentReport : BaseFragment() {

    val customerlist = ArrayList<BusinessAgentReportModel>()
    val customerlist_showing = ArrayList<BusinessAgentReportModel>()
    lateinit var recycler_collreports: RecyclerView
    lateinit var collReportAdapte: Adapter_businessagentReport
    ...
    lateinit var txt_filter: TextView
    ...
    override fun onCreateView(
        ...
        (activity as HomeActivity).setActionBarTitle("Business Agent Report")
        ...
        txt_filter = view.findViewById(R.id.txt_filter) as TextView
        ...
        txt_filter.visibility=View.INVISIBLE
        txt_cr_col1.setText(resources.getString(R.string.agentname))
        txt_cr_col2.setText(resources.getString(R.string.commission))
        txt_cr_col3.setText(resources.getString(R.string.paid_commission))
        txt_disp_total.setText(resources.getString(R.string.tot_commission))
            ...
            override fun onTextChanged(
                ...
                if (text == "" || text == null) {
                    ...
                    if (customerlist_showing.size > 0) {
                        ...
                        collReportAdapte =
                            Adapter_businessagentReport(customerlist_showing, object : AdapterClickListener {
                                override fun onPositionClicked(view: View, position: Int) {
                                    if (view.getTag().equals("listitem")) {
                                        val bundle = Bundle()
                                        bundle.putString("agentid", customerlist_showing.get(position).getAgentId())
                                        doFragmentTransactionWithBundle(BusinessAgentReportDetail(), "agentdet", true, bundle)
...
//                mobile = mobile.toLowerCase(Locale.getDefault())
        ...
        if (customerlist_showing.size > 0) {
            recycler_collreports.setVisibility(View.VISIBLE)
            collReportAdapte = Adapter_businessagentReport(customerlist_showing, object : AdapterClickListener {
                override fun onPositionClicked(view: View, position: Int) {
                    if (view.getTag().equals("listitem")) {
                        val bundle = Bundle()
                        bundle.putString("agentid", customerlist_showing.get(position).getAgentId())
    ...
    fun get_Collectionlist(
        ...
        if (checkForInternet()) {
            showProgressDialog()
            println("fetching")
            var resultlist = java.util.ArrayList<BusinessAgentReportModel>()
    ...
    private fun integrateList(leadslist: ArrayList<BusinessAgentReportModel>) {
        if (leadslist.size > 0) {
            customerlist.clear()
            customerlist_showing.clear()
            ...
            try {
                ...
                for (i in customerlist_showing) {
                    total += Constants.isEmtytoZero(i.getTotalCommissionAmount()!!).toInt()
                }
                    ...
                    response.isSuccessful -> {
                        ...
                        when {
                            response.code().equals(200) -> {
                                ...
                                doFragmentTransactionWithBundle(ViewReceiptPreview(), "agentdet", true, bundle)
                            }
                        }
                    }
                    else -> hideProgressDialog()
