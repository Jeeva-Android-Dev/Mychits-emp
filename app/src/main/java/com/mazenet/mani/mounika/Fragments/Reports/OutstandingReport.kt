...
import com.mazenet.mani.mounika.Adapters.ShowOutsReportList
import com.mazenet.mani.mounika.Fragments.BaseFragment
import com.mazenet.mani.mounika.Fragments.Collection.GroupFragment
import com.mazenet.mani.mounika.Fragments.Collection.ViewReceiptPreview
import com.mazenet.mani.mounika.Model.*
...
class OutstandingReport : BaseFragment() {

    val customerlist = ArrayList<OutstandingReportModel>()
    val customerlist_showing = ArrayList<OutstandingReportModel>()
    lateinit var recycler_collreports: RecyclerView
    lateinit var collReportAdapte: ShowOutsReportList
    ...
    override fun onCreateView(
        ...
        txt_cr_col2.setText(resources.getString(R.string.collected))
        txt_cr_col3.setText(resources.getString(R.string.tobecollected))
        txt_disp_total.setText(resources.getString(R.string.tot_outs))
            ...
            override fun onTextChanged(
                ...
                if (text == "" || text == null) {
                    ...
                    if (customerlist_showing.size > 0) {
                        ...
                        collReportAdapte =
                            ShowOutsReportList(customerlist_showing, object : AdapterClickListener {
                                override fun onPositionClicked(view: View, position: Int) {
                                    if (view.getTag().equals("listitem")) {
                                        ...
                                        bundle.putString(
                                            "cust_code",
                                            customerlist_showing.get(position).getCustomerCode().toString()
                                        )
                                        bundle.putString(
                                            "cust_mobile",
                                            customerlist_showing.get(position).getCustomerMobileNo().toString()
                                        )
                                        bundle.putString(
                                            "cust_phone",
                                            customerlist_showing.get(position).getCustomerPhoneNo().toString()
                                        )
...
//                mobile = mobile.toLowerCase(Locale.getDefault())
        ...
        if (customerlist_showing.size > 0) {
            recycler_collreports.setVisibility(View.VISIBLE)
            collReportAdapte = ShowOutsReportList(customerlist_showing, object : AdapterClickListener {
                override fun onPositionClicked(view: View, position: Int) {
                    if (view.getTag().equals("listitem")) {
                        val bundle: Bundle = Bundle()
                        bundle.putString(
    ...
    fun get_Collectionlist(
        ...
        if (checkForInternet()) {
            showProgressDialog()
            println("fetching")
            var resultlist = java.util.ArrayList<OutstandingReportModel>()
            ...
            val LeadListRequest = getleadslist.get_outs_reports(loginparameters)
            LeadListRequest.enqueue(object : Callback<ArrayList<OutstandingReportModel>> {

                override fun onFailure(call: Call<ArrayList<OutstandingReportModel>>, t: Throwable) {
                    hideProgressDialog()
                ...
                override fun onResponse(
                    call: Call<ArrayList<OutstandingReportModel>>,
                    response: Response<ArrayList<OutstandingReportModel>>
                ) {
                    hideProgressDialog()
                    when {
    ...
    private fun integrateList(leadslist: ArrayList<OutstandingReportModel>) {
        if (leadslist.size > 0) {
            customerlist.clear()
            customerlist_showing.clear()
            for(i in 0 until leadslist.size){
                if(leadslist[i].getTotalPending()!="0"){
                    customerlist.add(leadslist[i])
                    customerlist_showing.add(leadslist[i])
                }
            }
            ...
            try {
                ...
                for (i in customerlist_showing) {
                    total += Constants.isEmtytoZero(i.getTotalPending()!!).toInt()
                }
    ...
    fun showfilter(activity: Activity) {
        ...
        daterange_layout.visibility = View.GONE
